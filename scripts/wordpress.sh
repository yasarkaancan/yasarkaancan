...
echo "Enter database name:" && read db_name
echo "Enter database username:" && read db_username
echo "Enter database password:" && read db_password
echo "Enter database host (default is localhost):" && read db_host
...
echo "Enter folder name to install wordpress on ( Leave empty for here ):" && read folder_name
...
# Check the mysql user and password is correct
   ...
   if [ $? -ne 0 ]; then
       ...
       echo "Do you want to create a new mysql user ? (y/n)" && read create_user

       if [[ $create_user =~ ^([yY][eE][sS]|[yY])$ ]]
       then
       echo "Enter new mysql username:" && read db_username
       echo "Enter new mysql password:" && read db_password
       echo "Enter the mysql root password:" && read root_password
